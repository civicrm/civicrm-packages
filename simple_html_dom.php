...
class simple_html_dom_node
{
    ...
    /**
     * simple_html_dom_node constructor.
     *
     * @param $dom
     */
    ...
    protected function seek($selector, &$ret, $lowercase=false)
    {
        ...
        for ($i=$this->_[HDOM_INFO_BEGIN]+1; $i<$end; ++$i) {
            ...
            if ($pass && $key && $val  && $val!=='*') {
                ...
                if (is_object($debugObject)) {$debugObject->debugLog(2, 'testing node: ' . $node->tag . ' for attribute: '
...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    public function convert_text($text)
    {
        ...
        if (is_object($debugObject)) {$debugObject->debugLog(3, 'source charset: ' . $sourceCharset . ' target charaset: '
                                                                . $targetCharset);}
